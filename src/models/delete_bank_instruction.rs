/*
 * IB REST API
 *
 * The IB REST API reference documentation
 *
 * The version of the OpenAPI document: 2.7.0
 * Contact: api@interactivebrokers.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DeleteBankInstruction {
    #[serde(rename = "clientInstructionId")]
    pub client_instruction_id: String,
    #[serde(rename = "accountId")]
    pub account_id: String,
    #[serde(rename = "bankInstructionName")]
    pub bank_instruction_name: String,
    #[serde(rename = "bankInstructionMethod")]
    pub bank_instruction_method: BankInstructionMethod,
    #[serde(rename = "currency")]
    pub currency: String,
}

impl DeleteBankInstruction {
    pub fn new(client_instruction_id: String, account_id: String, bank_instruction_name: String, bank_instruction_method: BankInstructionMethod, currency: String) -> DeleteBankInstruction {
        DeleteBankInstruction {
            client_instruction_id,
            account_id,
            bank_instruction_name,
            bank_instruction_method,
            currency,
        }
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum BankInstructionMethod {
    #[serde(rename = "WIRE")]
    Wire,
    #[serde(rename = "ACH")]
    Ach,
}

impl Default for BankInstructionMethod {
    fn default() -> BankInstructionMethod {
        Self::Wire
    }
}

