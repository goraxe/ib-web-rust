/*
 * IB REST API
 *
 * The IB REST API reference documentation
 *
 * The version of the OpenAPI document: 2.9.0
 * Contact: api@interactivebrokers.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TraditionalBankInstructionVerification {
    #[serde(rename = "clientInstructionId")]
    pub client_instruction_id: String,
    #[serde(rename = "bankInstructionCode")]
    pub bank_instruction_code: BankInstructionCode,
    #[serde(rename = "bankInstructionName")]
    pub bank_instruction_name: String,
    #[serde(rename = "accountId")]
    pub account_id: String,
    #[serde(rename = "pendingInstructionId")]
    pub pending_instruction_id: f64,
    #[serde(rename = "creditAmount1")]
    pub credit_amount1: f64,
    #[serde(rename = "creditAmount2")]
    pub credit_amount2: f64,
}

impl TraditionalBankInstructionVerification {
    pub fn new(client_instruction_id: String, bank_instruction_code: BankInstructionCode, bank_instruction_name: String, account_id: String, pending_instruction_id: f64, credit_amount1: f64, credit_amount2: f64) -> TraditionalBankInstructionVerification {
        TraditionalBankInstructionVerification {
            client_instruction_id,
            bank_instruction_code,
            bank_instruction_name,
            account_id,
            pending_instruction_id,
            credit_amount1,
            credit_amount2,
        }
    }
}
/// 
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum BankInstructionCode {
    #[serde(rename = "USACH")]
    Usach,
    #[serde(rename = "CAACH")]
    Caach,
    #[serde(rename = "ACHUS")]
    Achus,
    #[serde(rename = "WIRE")]
    Wire,
}

impl Default for BankInstructionCode {
    fn default() -> BankInstructionCode {
        Self::Usach
    }
}

